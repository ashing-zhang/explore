...
function paginate<T>(rows: T[], page: number, pageSize: number): { page: number; pageSize: number; total: number; rows: T[] } {
...
export class DashboardPagesService {
  async getOrders(params: {
    ...
    const statuses: DashboardOrdersResponse['table']['rows'][number]['status'][] = ['已确认', '已完成', '已取消'];

    const totalRows = 52 + randInt(r, -6, 10);
    const allRows: DashboardOrdersResponse['table']['rows'] = Array.from({ length: totalRows }, (_, idx) => {
  ...
  async getMarket(params: {
    ...
    const points: DashboardMarketResponse['charts']['competitorScatter']['points'] = Array.from({ length: 18 }, (_, i) => {
    ...
    const tableRows: DashboardMarketResponse['table']['rows'] = points
      .filter((p) => !p.isSelf)
      .slice(0, 6)
      .map((p) => {
  ...
  async getStrategy(params: { dataDate?: string; page?: number; pageSize?: number }): Promise<DashboardStrategyResponse> {
    const today = new Date();
    const dataDate = parseIsoDate(params.dataDate, today);
    const seed = stableHash(['strategy', toIsoDate(dataDate)].join('|'));
    const r = mulberry32(seed);

    const cards: DashboardStrategyResponse['cards'] = [
      {
        id: 'S1',
        level: '高优先级',
        title: '工作日出清建议',
    ...
    const levels: DashboardStrategyResponse['history']['rows'][number]['level'][] = ['高优先级', '中优先级', '低优先级'];
    const statuses: DashboardStrategyResponse['history']['rows'][number]['status'][] = ['已执行', '评估中', '未执行'];
    const strategies = ['价格下调', '价格上调', '库存出清', '渠道倾斜', '促销拉新'];
    const triggers = ['竞品降价', '库存高压', '热度升温', '窗口开启', '取消率上升'];
    const scopes = ['工作日', '周末', '全量', '特定房型', '特定渠道'];

    const historyRows: DashboardStrategyResponse['history']['rows'] = Array.from({ length: historyTotal }, (_, idx) => {
  ...
  async getExecution(params: { dataDate?: string; page?: number; pageSize?: number }): Promise<DashboardExecutionResponse> {
    const today = new Date();
    const dataDate = parseIsoDate(params.dataDate, today);
    ...
    const statuses: DashboardExecutionResponse['table']['rows'][number]['status'][] = ['待执行', '执行中', '成功', '失败', '人工处理'];
    const actions = ['价格下调', '价格上调', '库存出清', '限售', '渠道倾斜'];
    const scopes = ['工作日', '周末', '全量', '特定房型', '特定渠道'];
    const operators = ['系统', '运营A', '运营B'];

    const tableTotal = 31;
    const allRows: DashboardExecutionResponse['table']['rows'] = Array.from({ length: tableTotal }, (_, idx) => {

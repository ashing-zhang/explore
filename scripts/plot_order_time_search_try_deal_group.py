...
运行指南：
    请在项目根目录下以模块方式运行：
    python -m scripts.plot_order_time_search_try_deal_group

输出说明：
    - 读取 data/search_try_deal_event_social 目录下的所有 csv 文件
    - 针对每个 csv 绘制一张包含两个子图的 booking_date-quantity 折线图
    - 第一个子图包含 search_quantity 一条折线
    - 第二个子图包含 try_quantity、order_quantity 两条折线
    - 输出到 data/order_time_search_try_deal_group_plots/{csv_stem}.png
"""
...
TOP_LINE_COLUMNS = ("search_quantity",)
BOTTOM_LINE_COLUMNS = ("try_quantity", "order_quantity")
Row = dict[str, str]


@dataclass(frozen=True)
class PlotConfig:
    """Define the input, output, and plotting columns for grouped charts."""

    input_dir: Path
    output_dir: Path
    date_column: str
    top_line_columns: tuple[str, ...]
    bottom_line_columns: tuple[str, ...]


def build_default_config(project_root: Path) -> PlotConfig:
    """Build the default configuration for grouped plotting."""
    return PlotConfig(
        input_dir=project_root / "data" / "search_try_deal_event_social",
        output_dir=project_root / "data" / "order_time_search_try_deal_group_plots",
        date_column=DATE_COLUMN,
        top_line_columns=TOP_LINE_COLUMNS,
        bottom_line_columns=BOTTOM_LINE_COLUMNS,
    )
...
def build_series(
    rows: Iterable[Row],
    date_column: str,
    line_columns: Iterable[str],
) -> tuple[list[date], dict[str, list[float]]]:
    """Convert rows into sorted dates and series values."""
    dated_rows = sorted(rows, key=lambda row: parse_booking_date(row.get(date_column, "")))
    dates = [parse_booking_date(row.get(date_column, "")) for row in dated_rows]
    series = {
...
def save_group_plot(
    dates: list[date],
    top_series: dict[str, list[float]],
    bottom_series: dict[str, list[float]],
    output_path: Path,
    plot_name: str,
) -> None:
    """Save a two-panel line chart for one CSV file."""
    try:
    ...
    for column_name, values in top_series.items():
        top_ax.plot(dates, values, linewidth=1.8, marker="o", markersize=3.2, label=column_name)
    top_ax.set_title(f"Search Quantity ({plot_name})")
    top_ax.set_ylabel("quantity")
    top_ax.grid(True, axis="y", alpha=0.25)
    top_ax.legend()

    for column_name, values in bottom_series.items():
        bottom_ax.plot(dates, values, linewidth=1.8, marker="o", markersize=3.2, label=column_name)
    bottom_ax.set_title(f"Try And Order Quantity ({plot_name})")
    bottom_ax.set_xlabel("booking_date")
    bottom_ax.set_ylabel("quantity")
...
def process_csv_file(csv_path: Path, config: PlotConfig) -> None:
    """Read one CSV file, build two plot panels, and save the PNG file."""
    encoding = detect_csv_encoding(csv_path)
    fieldnames, rows = read_csv_rows(csv_path, encoding=encoding)
    ensure_required_columns(
        fieldnames,
        [config.date_column, *config.top_line_columns, *config.bottom_line_columns],
    )

    dates, top_series = build_series(rows, config.date_column, config.top_line_columns)
    _, bottom_series = build_series(rows, config.date_column, config.bottom_line_columns)
    output_path = config.output_dir / f"{csv_path.stem}.png"
    save_group_plot(dates, top_series, bottom_series, output_path=output_path, plot_name=csv_path.stem)
    logger.info("Saved plot: %s", output_path)


def process_all_csv_files(config: PlotConfig) -> None:
    """Process all CSV files and generate grouped plots under the output directory."""
...
def main() -> None:
    """Run the grouped plotting task with the default project configuration."""

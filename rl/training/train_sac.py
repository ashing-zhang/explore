"""Train a SAC agent on HotelBlockEnv.
...
def main() -> None:
    ...
    LOGGER.info("Training SAC using config=%s", str(config_path))
    model = SAC(
        ...
        tensorboard_log=str(dirs["logs_dir"] / "sac"),
    )
    model.learn(total_timesteps=int(config.sac.total_timesteps))

    model_path = dirs["model_dir"] / "trained_model.zip"
    model.save(str(model_path))
    LOGGER.info("Saved SAC model to %s", str(model_path))

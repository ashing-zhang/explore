"""Train an RLlib PPO agent on HotelBlockEnv.
...
def main() -> None:
    ...
    LOGGER.info("Training RLlib PPO using config=%s", str(config_path))
    for _ in range(max(int(config.ppo.total_timesteps) // 10_000, 1)):
        algo.train()

    out = outputs_root()
    out.mkdir(parents=True, exist_ok=True)
    checkpoint_dir = algo.save(str(out / "rllib_checkpoints"))
    LOGGER.info("Saved RLlib PPO checkpoint to %s", str(checkpoint_dir))
    ray.shutdown()
